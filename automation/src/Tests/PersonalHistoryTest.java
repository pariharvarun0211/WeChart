...
public class PersonalHistoryTest {
			...
			) throws Exception
	{
		...
		for (int j=0;j<data.size();j++)
		{
			...
			medicalHistoryPage.menuDemographics(driver).sendKeys(data.get(j));

			Thread.sleep(5000);
			List<WebElement> options = driver.findElements(By.xpath("//ul[@id='select2-search_diagnosis_personal_history-results']/li"));
			...
			for (i=0;i<options.size();i++)
			{
				...
				{
					medicalHistoryPage.menuDemographics(driver).clear();
				}
				else {
